...
class SettingViewController: UIViewController , UITableViewDataSource, UITableViewDelegate , MFMailComposeViewControllerDelegate,APVAdManagerDelegate,FADDelegate{
    ...
    override func viewDidLoad() {
        ...
        table.estimatedRowHeight = CGFloat(CELL_ROW_HEIGT_THIN)
        table.rowHeight = UITableView.automaticDimension
        size = CGSize(width: table.frame.size.width, height: table.frame.size.width*9/16)
        table.delegate = self
    ...
    func tableView(_ tableView: UITableView, heightForFooterInSection section: Int) -> CGFloat {
        if section == settingSectionTitle.count - 1{
            if ADApearFlg() {
