...
| 設定 | 設定 | テーマ・課金管理・通知 |
| 発見 | iTunesランキング + YouTube検索 | 8カ国チャート・YouTube動画検索 |
| 練習 | 練習ダッシュボード | 速度プリセット・区間リピート・ディクテーション・フラッシュカード |
...
- バックグラウンド再生（AVAudioSession .playback）
- 再生エンジン: `HighSpeedAudioPlayer`（AVAudioEngine ベース、0.5x〜50x 34段階）
- 再生速度変更（`SpeedSheetViewController` ボトムシート。プリセット + スライダー）
...
- AVSpeechSynthesizer（TTS）で問題文を読み上げ（言語自動切り替え）
- SFSpeechRecognizer でマイク入力して採点
- 不正解単語を WeakWordService へ自動登録
- 練習中もミニプレイヤーで音楽再生可能

**フラッシュカード（FlashCardViewController）— v5.x 追加**
- NLTokenizer で歌詞から品詞フィルタ（名詞・動詞・形容詞）
- iOS 18+ ネイティブ翻訳APIで表・裏カード自動生成
- 知っている / もう一度 でわからない単語を WeakWordService へ記録

**弱点単語リスト（WeakWordListViewController）— v5.x 追加**
- ディクテーション不正解・フラッシュカード「もう一度」で蓄積
- 一覧表示・個別削除・全削除
- リストから直接フラッシュカード練習に移行可能

**練習履歴（PracticeHistoryViewController / PracticeHistoryService）— v5.x 追加**
- 連続日数（ストリーク）・週間ドット・月次バーチャート
...
- HighSpeedAudioPlayer による指定区間のループ再生

**速度プリセット（PracticeViewController）**
- 0.5x〜50x の速度プリセットを横スクロールでワンタップ切り替え
- 現在の速度をカードでハイライト表示
...
✅ 完了
  ...
  ⑫ HighSpeedAudioPlayer 導入（0.5x〜50x、AVAudioPlayer を完全置換）
  ⑬ フラッシュカード（FlashCardViewController）
  ⑭ 弱点単語リスト（WeakWordListViewController + WeakWordService）
  ⑮ 練習履歴・連続日数バッジ（PracticeHistoryService + PracticeHistoryViewController）
  ⑯ 発見タブ（DiscoverViewController）スケルトンシマーローディング
  ⑰ Firebase Analytics 全画面実装（10画面・15+イベント）
  ⑱ AdMob 頻度制御（4曲以上 かつ 前回から3分以上）

🔴 すぐやる（収益・評価に直結）
  A. App Store スクリーンショット更新（フラッシュカード・練習履歴・50x速再生を全面に出す）
  B. ASO（キーワード・説明文見直し）
  C. オンボーディング表示条件の確認・調整

🟡 3ヶ月以内（収益改善）
  D. サブスクプランの価値向上（WhisperKit Large モデル解放など）
  E. Apple Music 楽曲の再生対応（MPMusicPlayerController 併用）
  F. PlayMusicViewController のコードリファクタリング（新旧UI統合）
  G. AdMob メディエーション導入（Meta/AppLovin/Pangle/Unity）

🟢 半年以内（成長施策）
  H. ウィジェット対応
  I. 練習シェアカード機能
  J. Spotlight サーチ対応
  K. スキャンタブの役割見直し（練習タブへの統合 or 削除）
```
